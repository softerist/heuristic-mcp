...
describe("CodebaseIndexer Coverage Maximizer", () => {
  ...
  it("Line 146: Worker initialization failure catch block", async () => {
    ...
    // Expect 2 args
    expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("Error indexing bad.js"), expect.anything());
  });
  ...
  it("Lines 603 & 612: indexAll pruning branches", async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("Pruned 1 deleted/excluded files"));
  ...
  it("Line 746 & 773: indexAll loop and call graph extraction error", async () => {
    ...
    extractCallDataMock.mockImplementation(() => { throw new Error("Parse Error"); });
    ...
    expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("Call graph extraction failed"));
  });

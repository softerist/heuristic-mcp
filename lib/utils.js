 ...
 * for ~2-4x speedup on 768-dim vectors. Current loop-unrolling is sufficient
 * for most use cases.
 */
...
export function smartChunk(content, file, config) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (shouldSplit && safeToSplit && currentChunk.length > 0) {
      ...
      for (let k = currentChunk.length - 1; k >= 0 && overlapTokensCount < overlapTokens; k--) {

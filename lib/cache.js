...
async function parseJsonInWorker(filePath) {
  return new Promise((resolve, reject) => {
    ...
    worker.once('exit', (code) => {
      if (code !== 0) {
        finish(reject, new Error(`JSON worker exited with code ${code}`));
      }
    });
  });
}
...
export class EmbeddingsCache {
  ...
  async load() {
    ...
    try {
      ...
      if (cacheData && hashData) {
        ...
        if (filtered > 0) {
          console.info(`[Cache] Filtered ${filtered} outdated cache entries`);
        }
  ...
  async loadAnnIndexFromDisk(HierarchicalNSW, dim) {
    ...
    this.annMeta = meta;
    this.annDirty = false;
    console.info(`[ANN] Loaded ANN index (${meta.count} vectors)`);
    return true;
  }
    ...
    try {
      ...
      if (this.config.annIndexCache !== false) {
        try {
          ...
          console.info(`[ANN] Saved ANN index (${total} vectors)`);
        } catch (error) {
  ...
  rebuildCallGraph() {
    ...
    import('./call-graph.js')
      ...
      .catch((err) => {
        console.warn(`[CallGraph] Failed to build: ${err.message}`);
        this.callGraph = null;
      });
  }

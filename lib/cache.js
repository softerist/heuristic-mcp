...
export class EmbeddingsCache {
  ...
  setVectorStore(store) {
    this.vectorStore = Array.isArray(store) ? store : [];
  ...
  getAnnVector(index) {
    return this.vectorStore[index]?.vector || null;
  }
  ...
  async loadAnnIndexFromDisk(HierarchicalNSW, dim) {
    ...
    if (this.config.verbose) {
      console.log(`[ANN] Loaded ANN index (${meta.count} vectors, cap=${meta.maxElements})`);
    }
    return true;
  }

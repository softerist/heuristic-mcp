...
export class EmbeddingsCache {
    ...
    try {
      ...
      if (useBinary) {
        try {
          ...
          cacheData = this.binaryStore.toChunkViews({
  ...
  getChunkVector(chunk, index = null) {
    ...
    if (this.binaryStore && Number.isInteger(resolved)) {
      return this.binaryStore.getVector(resolved);
    }
    return null;
  }

  getChunkContent(chunk, index = null) {
    if (chunk?.content !== undefined && chunk?.content !== null) {
      return chunk.content;
    }
    const resolved = Number.isInteger(index) ? index : chunk?._index;
    if (this.binaryStore && Number.isInteger(resolved)) {
      return this.binaryStore.getContent(resolved);
    }
    return '';
  }
  ...
  removeFileFromStore(file) {
    ...
    let w = 0;
    const updateIndex = !this.binaryStore;

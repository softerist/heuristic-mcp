...
export class EmbeddingsCache {
  ...
  async performSave() {
    ...
    try {
      ...
      const vectorStore = Array.isArray(this.vectorStore) ? this.vectorStore : [];
      this.cacheMeta = {
        version: CACHE_META_VERSION,
      ...
      if (this.config.vectorStoreFormat === 'binary') {
        ...
        try {
          for (let i = 0; i < total; i++) {
            const pending = vectorWriter.writeItem(vectorStore[i]);
  ...
  /**
   * Returns an array of all file paths currently in the hash map.
   * @returns {string[]}
   */
  getFileHashKeys() {
    return Array.from(this.fileHashes.keys());
  ...
  /**
   * Sets the entire file hash map.
   * @param {Map<string, string|object>} map
   */
  setFileHashes(map) {
    this.fileHashes = map instanceof Map ? map : new Map(Object.entries(map || {}));
  }
  ...
  getChunkVector(chunk, index = null) {
    if (typeof chunk === 'number') {
      ...
      if (!store && this.binaryStore) {
        return this.binaryStore.getVector(chunk);
      }
      return null;
    }
  ...
  async queryAnn(queryVector, k) {
    if (!Array.isArray(this.vectorStore) || this.vectorStore.length === 0) return null;

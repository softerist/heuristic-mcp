...
const HNSWLIB_ERROR_RESET_MS = 5 * 60 * 1000;
const ANN_DIMENSION_SAMPLE_SIZE = 100;
const DEFAULT_READER_WAIT_TIMEOUT_MS = 5000;
...
async function readJsonFile(
  filePath,
  { workerThresholdBytes = DEFAULT_JSON_WORKER_THRESHOLD } = {}
) {
  let stats;
  try {
  ...
  async ensureAnnIndex() {
    ...
    this.annLoading = (async () => {
      try {
        ...
        for (let i = 1; i < Math.min(ANN_DIMENSION_SAMPLE_SIZE, this.vectorStore.length); i++) {
        ...
        if (dimensionMismatch) {
          this.initErrors.push({
            stage: 'ensureAnnIndex',
            message: `Vector dimension inconsistency detected. Expected ${dim}. Full reindex required.`,
          });

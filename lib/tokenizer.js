...
const MODEL_TOKEN_LIMITS_RAW = {
  ...
  // allocates O(n²) memory for attention (~14GB at 8192 tokens). Using 512 tokens
  // keeps memory under 2GB while still providing good semantic understanding.
  ...
  function getModelTokenLimitFromLower(lowerName, originalName) {
    ...
    // Heuristics for common models (use conservative limits for ONNX memory safety)
    // Note: Even though these models support 8192 tokens, ONNX runtime O(n²) memory
    // makes that impractical. Using 512 tokens for stability.

...
export const MODEL_TOKEN_LIMITS = Object.freeze({
  'jinaai/jina-embeddings-v2-base-code': 8192,
  default: 8192,
});

const DEFAULT_LIMIT = MODEL_TOKEN_LIMITS.default ?? 8192;
...
function getModelTokenLimitFromLower(lowerName, originalName) {
  ...
  return MODEL_LIMITS_LC.get(lowerName) ?? DEFAULT_LIMIT;
}
...
export function getModelTokenLimit(modelName) {
  ...
  return MODEL_LIMITS_LC.get(modelName.toLowerCase()) ?? DEFAULT_LIMIT;
}
...
export function exceedsTokenLimit(text, modelName) {
  return estimateTokens(text) > getModelTokenLimit(modelName);
}

...
async function resolveWorkspaceDir(workspaceDir) {
  ...
  // Guard: warn if CWD is an IDE/system directory, not a user project
  if (isNonProjectDirectory(cwd)) {
    console.warn(
      `[Config] ⚠️  Workspace resolved to CWD "${cwd}" which looks like an IDE/system directory, not a user project.`
    );
    console.warn(
      '[Config] ⚠️  Set HEURISTIC_MCP_WORKSPACE env var or pass --workspace <path> to fix this.'
    );
  }

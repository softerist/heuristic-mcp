...
export async function loadConfig(workspaceDir = null) {
  try {
    ...
    if (workspaceDir) {
      ...
      console.log(`[Config] Workspace mode: ${baseDir}`);
    } else {
    ...
    if (userConfig.cacheDirectory) {
      ...
      try {
        ...
        if (stats.isDirectory()) {
          ...
          if (config.verbose) {
            console.log(`[Config] Using existing local cache: ${legacyPath}`);
          }
        }
      } catch {
    ...
    if (config.smartIndexing !== false) {
      ...
      if (detectedTypes.length > 0) {
        ...
        console.log(`[Config] Smart indexing: ${detectedTypes.join(', ')}`);
        console.log(`[Config] Applied ${smartPatterns.length} smart ignore patterns`);
      } else {
        console.log('[Config] No project markers detected, using default patterns');
      }
    }
  ...
  if (process.env.SMART_CODING_EMBEDDING_MODEL !== undefined) {
    ...
    if (value.length > 0) {
      ...
      console.log(`[Config] Using custom embedding model: ${value}`);
    }
  }

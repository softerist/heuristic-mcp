...
const DEFAULT_CONFIG = {
  ...
  embeddingPoolIdleTimeoutMs: 600000, // DEFAULT_MEMORY_CLEANUP_CONFIG.embeddingPoolIdleTimeoutMs,
  ...
  recycleServerOnHighRssCooldownMs: DEFAULT_MEMORY_CLEANUP_CONFIG.recycleServerOnHighRssCooldownMs,
...
function applyAllNamespaces(targetConfig, sourceConfig) {
  for (const namespace of CONFIG_NAMESPACES) {
    applyNamespace(targetConfig, sourceConfig, namespace.name, namespace.keys, namespace.defaults);
  }
}
...
export async function loadConfig(workspaceDir = null) {
  ...
  if (
    ...
    if (!isNaN(value) && value > 0) {
      ...
      config.embeddingProcessGcRssThresholdMb = DEFAULT_CONFIG.embeddingProcessGcRssThresholdMb;
    }
  }
  ...
  if (
    ...
    if (!isNaN(value) && value >= 0) {
      ...
      config.embeddingProcessGcMinIntervalMs = DEFAULT_CONFIG.embeddingProcessGcMinIntervalMs;
    }
  }

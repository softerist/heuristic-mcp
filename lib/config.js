...
async function resolveWorkspaceDir(workspaceDir) {
  ...
  // Codex can expose workspace info under additional CODEX_* env var names.
  // Probe those dynamically so we still resolve the active project workspace
  // without requiring hardcoded --workspace in config.

...
const DEFAULT_CONFIG = {
  ...
  excludePatterns: [
    ...
    // Development scripts (project-specific)
    '**/scripts/**',
    '**/tools/**',
  ],
  ...
  cacheCleanup: {
    ...
    workspaceGraceDays: 7, // Days grace period for missing workspace (unmounted drives)
...
function applyMemoryCleanupNamespace(targetConfig, sourceConfig = null) {
  const sourceNamespace =
    sourceConfig && typeof sourceConfig.memoryCleanup === 'object' ? sourceConfig.memoryCleanup : {};
  const mergedNamespace = {
    ...DEFAULT_CONFIG.memoryCleanup,
    ...(targetConfig.memoryCleanup && typeof targetConfig.memoryCleanup === 'object'
      ? targetConfig.memoryCleanup
      : {}),
  };

  for (const key of MEMORY_CLEANUP_KEYS) {
  ...
  targetConfig.memoryCleanup = mergedNamespace;
}

function syncMemoryCleanupNamespace(targetConfig) {
  const currentNamespace =
    targetConfig.memoryCleanup && typeof targetConfig.memoryCleanup === 'object'
      ? targetConfig.memoryCleanup
      : {};
  const mergedNamespace = { ...DEFAULT_CONFIG.memoryCleanup, ...currentNamespace };
  for (const key of MEMORY_CLEANUP_KEYS) {
    mergedNamespace[key] = targetConfig[key];
  }
  targetConfig.memoryCleanup = mergedNamespace;
...
export async function loadConfig(workspaceDir = null) {
  try {
    ...
    applyMemoryCleanupNamespace(config, userConfig);
  ...
  syncMemoryCleanupNamespace(config);
  return config;
}

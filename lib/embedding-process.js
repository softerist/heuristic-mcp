...
const log = (...args) => {
  if (currentRequestId > 1 && !process.env.EMBEDDING_PROCESS_VERBOSE) {
    return;
  }
  console.error(...args);
};
...
async function runEmbedding(payload) {
  ...
  for (let batchStart = 0; batchStart < chunks.length; batchStart += BATCH_SIZE) {
    ...
    try {
      ...
      log(`[Child:${process.pid}] Batch failed, falling back to single: ${error.message}`);
    ...
    if (batchEnd % 20 === 0 || batchEnd === chunks.length) {
      ...
      log(`[Child:${process.pid}] Request ${reqId}: processed ${batchEnd}/${chunks.length} chunks in ${elapsed}s, ${formatMemory()}`);
    }
    ...
    if (batchEnd % 100 === 0 && typeof global.gc === 'function') {
      global.gc();
      const elapsed = ((Date.now() - start) / 1000).toFixed(1);
      log(`[Child:${process.pid}] Request ${reqId}: GC triggered after ${batchEnd}/${chunks.length} chunks in ${elapsed}s, ${formatMemory()}`);
    }
  }

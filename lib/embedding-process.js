...
async function runEmbedding(payload) {
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    try {
      ...
      // CRITICAL: Deep copy the data BEFORE disposing the tensor
      const vector = new Float32Array(output.data);

      // Properly dispose the tensor to release ONNX runtime memory
      // @huggingface/transformers v3 tensors have dispose() method

...
let ONNX = null;
try {
  const backend = await import('@huggingface/transformers/src/backends/onnx.js');
...
export function getNativeOnnxStatus() {
  ...
  if (ortVersion && expectedOrtVersion) {
    ...
    const expectedParts = String(expectedOrtVersion).replace(/^[^0-9]*/, '').split('.');
...
export function configureNativeOnnxBackend({ log, label, threads } = {}) {
  ...
  if (!status.available) {
    if (log) {
      ...
      log(`${label ? `${label} ` : ''}Native ONNX backend unavailable: ${msg}. Falling back to WASM.`);
    }
    return false;
  }
  ...
  if (normalizedThreads) {
    ...
    if (log) {
      ...
      log(`${label ? `${label} ` : ''}ONNX session threads set (intraOp=${intra}, interOp=${inter})`);
    }
  }

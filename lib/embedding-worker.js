...
async function prepareFileContent(file, providedContent, maxFileSize) {
  ...
  try {
    ...
    // Post-read TOCTOU mitigation: verify actual content size doesn't exceed limit
    const actualSize = Buffer.byteLength(content, 'utf-8');
    if (actualSize > maxFileSize) {
      return { status: 'skipped', reason: 'too_large_after_read', mtimeMs, size: actualSize };
    }
    size = actualSize; // Update size to actual content size
  } catch (err) {

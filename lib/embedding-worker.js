 ...
 * If embeddingDimension is null/undefined, returns the original vector unchanged.
...
async function processFileTask(message) {
  ...
  if (!message.content) {
    try {
      ...
      // NOTE: TOCTOU race exists between stat and readFile - file could grow after
      // this check. Acceptable since we catch memory errors and limit is a soft hint.
  ...
  try {
    content =
      typeof message.content === 'string' ? message.content : await fs.readFile(file, 'utf-8');
  } catch (err) {
        ...
        try {
          ...
          if (!fileMsg.content) {
            try {
              ...
              status = 'skipped';
              reason = `stat_failed: ${err.message}`;
            }
          } else {
            // Content provided
          ...
          if (status === 'processing') {
            ...
            if (content === null) {
              try {
                ...
                status = 'skipped';
                reason = `read_failed: ${err.message}`;
              }
            }
          }
          ...
          if (status === 'processing') {
            ...
            // E. Chunking - merge configs properly to avoid empty-object bypass
            const chunkConfig = {
              ...(workerData.config || {}),
            ...
            // Trigger GC every 100 files (was 5)
